...
(function (window, undefined) {
  ...
  const core_push = Array.prototype.push;
  const core_slice = Array.prototype.slice;
  const core_indexOf = Array.prototype.indexOf;
  const core_toString = Object.prototype.toString;
  const core_hasOwn = Object.prototype.hasOwnProperty;
  ...
  jQuery.extend({
    ...
    each(obj, callback, args) {
      ...
      let i = 0;
      const { length } = obj;
      const isObj = length === undefined || jQuery.isFunction(obj);
 ...
 */
  ...
  jQuery.support = (function () {
    ...
    jQuery(() => {
      ...
      const divReset = 'padding:0;margin:0;border:0;display:block;overflow:hidden;';
  ...
  jQuery.fn.extend({
    data(key, value) {
      let parts; let part; let attr; let name; let l;
      const elem = this[0];
      let i = 0;
      let data = null;
  ...
  let nodeHook; let boolHook; let fixSpecified;
  const rclass = /[\t\r\n]/g;
  const rreturn = /\r/g;
  ...
  jQuery.fn.extend({
    ...
    removeClass(value) {
      let removes; let className; let elem; let c; let cl; let i; let
        l;
    ...
    toggleClass(value, stateVal) {
      ...
      return this.each(function () {
        if (type === 'string') {
          ...
          const self = jQuery(this);
          let state = stateVal;
          const classNames = value.split(core_rspace);
  ...
  jQuery.extend({
    ...
    removeAttr(elem, value) {
      let propName; let attrNames; let name; let isBool;
      let i = 0;
 ...
 */
  jQuery.event = {
    ...
    add(elem, types, handler, data, selector) {
      ...
      let handleObjIn; let handlers; let
        special;
    ...
    trigger(event, data, elem, onlyHandlers) {
      ...
      let cache; let exclusive; let i; let cur; let old; let ontype; let special; let handle; let eventPath; let bubbleType;
    ...
    dispatch(event) {
      ...
      let i; let j; let cur; let ret; let selMatch; let matched; let matches; let handleObj; let sel; let related;
 ...
 */
  (function (window, undefined) {
    ...
    const indexOf = [].indexOf || function (elem) {
      let i = 0;
    ...
    const pseudos = `:(${characterEncoding})(?:\\((?:(['"])((?:\\\\.|[^\\\\])*?)\\2|([^()[\\]]*|(?:(?:${attributes})|[^:]|\\\\.)*|.*))\\)|)`;

    // For matchExpr.POS and matchExpr.needsContext
    const pos = `:(even|odd|eq|gt|lt|nth|first|last)(?:\\(${whitespace
		 }*((?:-\\d)?\\d*)${whitespace}*\\)|)(?=[^-]|$)`;
    ...
    const assertTagNameNoComments = assert((div) => {
      div.appendChild(document.createComment(''));
      return !div.getElementsByTagName('*').length;
    });

    // Check if getAttribute returns normalized href attributes
    const assertHrefNotNormalized = assert((div) => {
    ...
    const assertAttributes = assert((div) => {
    ...
    const assertUsableClassName = assert((div) => {
    ...
    const assertUsableName = assert((div) => {
      // Inject content
      div.id = expando + 0;
    ...
    function Sizzle(selector, context, results, seed) {
      results = results || [];
      context = context || document;
      let match; let elem; let xml; let m;
 ...
 */
    ...
    function tokenize(selector, parseOnly) {
      let matched; let match; let tokens; let type; let soFar; let groups; let preFilters;
      ...
      // Check against all ancestor/preceding elements
        : function (elem, context, xml) {
          ...
          if (!xml) {
            ...
            const cachedkey = dirkey + cachedruns;
            while ((elem = elem[dir])) {
              if (checkNonElements || elem.nodeType === 1) {
    ...
    function setMatcher(preFilter, selector, matcher, postFilter, postFinder, postSelector) {
      ...
      return markFunction((seed, results, context, xml) => {
        ...
        let i; let elem; let postFilterIn;
        const preMap = [];
        const postMap = [];
        const preexisting = results.length;

        // Get initial elements from seed or context
        const elems = seed || multipleContexts(selector || '*', context.nodeType ? [context] : context, [], seed);
    ...
    if (document.querySelectorAll) {
      (function () {
        ...
        let rbuggyMatches = [':active', ':focus'];
        const matches = docElem.matchesSelector
        ...
        select = function (selector, context, results, seed, xml) {
          ...
          if (!seed && !xml && (!rbuggyQSA || !rbuggyQSA.test(selector))) {
            ...
            let newSelector = context.nodeType === 9 && selector;
  ...
  jQuery.fn.extend({
    find(selector) {
      let i; let l; let length; let n; let r; let ret;
      const self = this;
  ...
  jQuery.fn.extend({
    ...
    domManip(args, table, callback) {
      ...
      let results; let first; let fragment; let iNoClone;
      let i = 0;
      const value = args[0];
      const scripts = [];
      const l = this.length;
  ...
  jQuery.buildFragment = function (args, context, scripts) {
    let fragment; let cacheable; let cachehit;
    const first = args[0];
  ...
  jQuery.extend({
    ...
    clean(elems, context, fragment, scripts) {
      let i; let j; let elem; let tag; let wrap; let depth; let div; let hasBody; let tbody; let len; let handleScript; let jsTags;
  ...
  let curCSS; let iframe; let iframeDoc;
  ...
  const eventsToggle = jQuery.fn.toggle;
  ...
  if (window.getComputedStyle) {
    curCSS = function (elem, name) {
      let ret; let width; let minWidth; let maxWidth;
  ...
  function getWidthOrHeight(elem, name, extra) {
    ...
    const isBorderBox = jQuery.support.boxSizing && jQuery.css(elem, 'boxSizing') === 'border-box';
  ...
  function addToPrefiltersOrTransports(structure) {
    ...
    return function (dataTypeExpression, func) {
      ...
      let dataType; let list; let placeBefore;
      const dataTypes = dataTypeExpression.toLowerCase().split(core_rspace);
      let i = 0;
 ...
 */
  ...
  jQuery.ajaxPrefilter('json jsonp', (s, originalSettings, jqXHR) => {
    ...
    const { data } = s;
    const { url } = s;
    const hasCallback = s.jsonp !== false;
    const replaceInUrl = hasCallback && rjsonp.test(url);
    const replaceInData = hasCallback && !replaceInUrl && typeof data === 'string'
  ...
  function Animation(elem, properties, options) {
    ...
    var tick = function () {
      ...
      const percent = 1 - (remaining / animation.duration || 0);
      let index = 0;
  ...
  function defaultPrefilter(elem, props, opts) {
    let index; let prop; let value; let length; let dataShow; let tween; let hooks; let oldfire;
    const anim = this;
    const { style } = elem;
    const orig = {};
    const handled = [];
    const hidden = elem.nodeType && isHidden(elem);
  ...
  jQuery.fn.offset = function (options) {
    ...
    let docElem; let body; let win; let clientTop; let clientLeft; let scrollTop; let scrollLeft;
