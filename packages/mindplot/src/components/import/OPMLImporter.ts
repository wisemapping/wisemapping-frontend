...
class OPMLImporter extends Importer {
  ...
  import(nameMap: string, description?: string): Promise<string> {
    ...
    console.log(`Importing OPML map: ${nameMap}, description: ${description}`);
    console.log('Map content:', this.opmlInput);
    return Promise.resolve(`<map name="${nameMap}"><node TEXT="OPML Map"></node></map>`);
  }
}
