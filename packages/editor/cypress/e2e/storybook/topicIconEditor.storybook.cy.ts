...
describe('TopicIconEditor Storybook', () => {
  const storybookUrl = 'http://localhost:6008/iframe.html?id=editor-topiciconeditor--default&viewMode=story';
  ...
  it('should trigger onIconChange action when switching to image mode and selecting an image', () => {
    ...
    cy.get('img, [role="button"]').first().click();
  });
  ...
  it('should display existing emoji in WithEmoji story', () => {
    cy.visit('http://localhost:6008/iframe.html?id=editor-topiciconeditor--with-emoji&viewMode=story');
  ...
  it('should display existing image in WithImage story', () => {
    cy.visit('http://localhost:6008/iframe.html?id=editor-topiciconeditor--with-image&viewMode=story');
